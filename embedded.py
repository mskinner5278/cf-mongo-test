...
def insert_property(channel, name, owner, value):
    ...
    if (not name in props):
        ...
        prop_id = next_prop
        db.property.insert({"name": name,"owner":owner})  #ADD  {id,name,owner, is_tag}
    else:
        prop_id = props[name]
    prop_list = prop_list + [{"name": name, "owner": owner, "value": value, "properties._id": db.property.find({"name": name}).next()["_id"]}]
    #return [{"name": name, "owner": owner, "value": value, "properties._id": db.property.find({"name": name}).next()["_id"]}]
...
def insert_tag(channel,name, owner):
    ...
    if (not name in props):
        ...
        props[name] = next_prop
        prop_id = next_prop
        db.tags.insert({"name":name, "owner":owner})
        ###########################db.tags
    else:
        prop_id = props[name]
    tag_list = tag_list + [ db.tags.find({"name":name})[0] ]
    #########################db.prop_vals
...
def create_db():
    ...
    db.channels.create_index([("properties.name",1),("properties.value",1)])
